...
function footnoteDefinition(node, _, context, safeOptions) {
  ...
  tracker.shift(4);
  value += tracker.move(
    indentLines(containerFlow(node, context, tracker.current()), map$2)
  );
  exit();
...
(function (JsonLayout) {
    JsonLayout["AoS"] = "AoS";
    JsonLayout["SoA"] = "SoA";
})(exports.JsonLayout || (exports.JsonLayout = {}));
class MarkdownTable2Json {
    buffer;
    layout;
    minify;
    frozenProccesor;
    ast;
    constructor({ markdownString, layout, minify }) {
        this.buffer = markdownString;
        this.layout = layout ?? exports.JsonLayout.SoA;
        this.minify = minify ?? true;
        this.frozenProccesor = unified().use(remarkParse).use(remarkGfm).use(remarkStringify);
        this.ast = this.frozenProccesor.parse(this.buffer);
    }
    transform() {
        this.dfs(this.ast, (node) => {
            if (this.layout === exports.JsonLayout.AoS)
                this.processTableAoS(node);
            if (this.layout === exports.JsonLayout.SoA)
                this.processTableSoA(node);
        });
        return this.toMarkdownString(this.ast);
    }
    prettyAst() {
        return node_util.inspect(this.ast);
    }
    toMarkdownString(node) {
        return this.frozenProccesor.stringify(node);
    }
    cellToString(node) {
        let buffer = "";
        this.dfs(node, (n) => {
    ...
    processTableAoS(node) {
        if (node.type !== "table")
            return;
        const table = node;
        if (table.children.length < 1)
            return;
        const headers = table.children[0].children.map((header) => this.cellToString(header));
        const json = [];
        for (const row of table.children.slice(1)) {
            const cells = row.children.map((cell) => this.cellToString(cell));
            const obj = {};
...
exports.MarkdownTable2Json = MarkdownTable2Json;
//# sourceMappingURL=mdt2json.cjs.map
